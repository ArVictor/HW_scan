...
class HW:
    def __init__(
        ...
        Parameters:
            ...
            # poisson_bracket_coeff (float, optional): Coefficient for the Arakawa scheme. Default is 1. RENAMED TO ARAKAWA_COEFF
            ...
            poisson_method (str, optional): Method to solve the Poisson equation. ALWAYS "fourier"!!! THIS IS A USELESS PARAMETER HERE JUST TO MATCH THE PARAMETERS FROM THE tf_hw2d VERSION!
        ...
        print("fourier_poisson_double:", fourier_poisson_double)
        ...
        self.poisson_bracket_coeff = arakawa_coeff
    ...
    def rk2_step(self, plasma: Namespace, dt: float, dx: float) -> Namespace:
        # RK4
        t0 = time.time()
        yn = plasma
    ...
    def get_phi(self, omega: np.ndarray, dx: float) -> np.ndarray:
        ...
        o_mean = np.mean(omega, axis=(-2, -1), keepdims=True)
    ...
    def gradient_2d(
        ...
        dy_p = self.gradient_func(phi, dx=dx, axis=-2) # works with a first batch dimension

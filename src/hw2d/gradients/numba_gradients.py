...
def laplace_N(arr: np.ndarray, dx: float, N: int) -> np.ndarray:
    ...
    """
    res_arr = laplace(arr, dx)
    for i in range(N-1):
        res_arr = laplace(res_arr, dx)
    return res_arr
...
def periodic_laplace_N(arr: np.ndarray, dx: float, N: int) -> np.ndarray:
    ...
    padding = (*((0, 0) for i in range(arr.ndim - 2)),(N, N),(N, N)) # we pad the last two dimensions: (y, x) not the first (batch)
    return laplace_N(np.pad(arr, padding, "wrap"), dx, N)

...
def fourier_poisson_double(tensor: np.ndarray, dx: float, times: int = 1) -> np.ndarray:
    ...
    for i in range(tensor.shape[0]):
        k = np.meshgrid(*[np.fft.fftfreq(int(n)) for n in tensor.shape[1:]], indexing="ij")
        k = np.stack(k, -1)
        k = np.sum(k**2, axis=-1)
        ...
        with np.errstate(divide="ignore", invalid="ignore"):
            result[i,...] = frequencies[i, ...] / (fft_laplace**times)
        result[i,...] = np.where(fft_laplace == 0, 0, result[i,...])

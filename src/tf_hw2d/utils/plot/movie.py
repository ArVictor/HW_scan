...
def create_movie(
    ...
    # VICTOR: In interactive mode this function crashes. It is a know bug of matplotlib: https://stackoverflow.com/questions/43776528/python-animation-figure-window-cannot-be-closed-automatically
    # Setup details
    plasma_steps = h5py.File(input_filename, "r")
    params = dict(plasma_steps.attrs)
    # params = {'L': 41.88790204786391, 'N': 3, 'age': 1000.0, 'arak': 1, 'c1': 1.0, 'dt': 0.05, 'dx': 0.6544984694978736, 'dy': 0.0818123086872342, 'frame_dt': 0.05, 'grid_pts': 64, 'gridsize': [512, 512], 'id': 'a975a27c551766f1643fbbdd65729b7919dff64313e4251d54d6ea8f3428cd12b35533553f068abe5cdc83a49679b18434e1575a2df9bc344ce2b9f3060e4318', 'k0': 0.15, 'kappa': 1, 'nu': 5e-06, 'scale': 0.01, 'tstart': 0, 'tstop': 1000.0, 'version': 3, 'x': 64, 'y': 64}
    print("params:", params)
    ...
    # Animation
    writer = animation.writers["ffmpeg"](fps=fps, metadata=dict(artist="VA"))
    ...
    # Save Movie
    output_filename = (
        f"{output_filename}_dpi{dpi}fps{fps:.0f}sp{speed:.0f}t0{t0}t1{t1}.mp4"
    )

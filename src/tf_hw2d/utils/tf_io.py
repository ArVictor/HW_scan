...
def save_to_buffered_h5(
    ...
    for idx, field in enumerate(field_list):
        n_dims = new_val[field].rank
        buffer[field][buffer_index] = (
            new_val[field]
            .numpy(dim_order[:n_dims])
            .reshape(buffer[field][buffer_index].shape)
        )
    buffer_index += 1
...
def continue_h5_file(
    ...
    data = math.Dict(**data, age=age, dx=params["dx"])
    params = {
        k: params[k]
